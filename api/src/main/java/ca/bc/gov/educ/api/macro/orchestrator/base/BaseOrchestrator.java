package ca.bc.gov.educ.api.macro.orchestrator.base;
...
import ca.bc.gov.educ.api.macro.struct.v1.NotificationEvent;
...
public abstract class BaseOrchestrator<T> implements EventHandler, Orchestrator {
  ...
  public BaseOrchestrator<T> begin(final EventType nextEvent, final SagaStep<T> stepToExecute) {
    return this.registerStepToExecute(EventType.INITIATED, EventOutcome.INITIATE_SUCCESS, (T sagaData) -> true, nextEvent, stepToExecute);
  }
  ...
  public BaseOrchestrator<T> begin(final Predicate<T> nextStepPredicate, final EventType nextEvent, final SagaStep<T> stepToExecute) {
    return this.registerStepToExecute(EventType.INITIATED, EventOutcome.INITIATE_SUCCESS, nextStepPredicate, nextEvent, stepToExecute);
  }
  ...
  public BaseOrchestrator<T> end(final EventType currentEvent, final EventOutcome outcome) {
    return this.registerStepToExecute(currentEvent, outcome, (T sagaData) -> true, EventType.MARK_SAGA_COMPLETE, this::markSagaComplete);
  }
  ...
  public BaseOrchestrator<T> end(final EventType currentEvent, final EventOutcome outcome, final SagaStep<T> stepToExecute) {
    return this.registerStepToExecute(currentEvent, outcome, (T sagaData) -> true, EventType.MARK_SAGA_COMPLETE, (Event event, Saga saga, T sagaData) -> {
   ...
   */
  protected SagaEventStates createEventState(@NotNull final Saga saga, @NotNull final EventType eventType, @NotNull final EventOutcome eventOutcome, final String eventPayload) {
   ...
   * @throws IOException          if there is connectivity problem
   */
  private void replayFromLastEvent(final Saga saga, final List<SagaEventStates> eventStates, final T t) throws InterruptedException, TimeoutException, IOException {
  ...
  private void replayFromBeginning(final Saga saga, final T t) throws InterruptedException, TimeoutException, IOException {
    final Event event = Event.builder()
        ...
        .build();
    this.findAndInvokeNextStep(saga, t, EventType.INITIATED, EventOutcome.INITIATE_SUCCESS, event);
  }
  ...
  private boolean sagaEventExecutionNotRequired(@NotNull final Event event) {
    return (event.getEventType() == EventType.INITIATED && event.getEventOutcome() == EventOutcome.INITIATE_SUCCESS && SELF.equalsIgnoreCase(event.getReplyTo()))
        || event.getEventType() == EventType.MARK_SAGA_COMPLETE && event.getEventOutcome() == EventOutcome.SAGA_COMPLETED;
  }
  ...
  private void broadcastSagaInitiatedMessage(@NotNull final Event event) {
    ...
    if (this.shouldSendNotificationEvent && event.getEventType() == EventType.INITIATED && event.getEventOutcome() == EventOutcome.INITIATE_SUCCESS
      ...
      notificationEvent.setSagaStatus(EventType.INITIATED.toString());
