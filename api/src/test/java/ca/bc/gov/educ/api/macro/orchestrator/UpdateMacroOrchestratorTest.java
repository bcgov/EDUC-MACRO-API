...
public class UpdateMacroOrchestratorTest {
  /**
   * The Repository.
   */
  @Autowired
  ...
  @Autowired
  private UpdateMacroOrchestrator orchestrator;
  ...
  public void setUp() throws JsonProcessingException {
    ...
    saga = sagaService.createSagaRecordInDB(SagaEnum.MACRO_UPDATE_SAGA.toString(), "Test",
  ...
  @Test
  public void testUpdateMacro_givenEventAndSagaData_shouldPostEventToMacroApi() throws IOException, TimeoutException, InterruptedException {
    ...
    assertThat(newEvent.getEventType()).isEqualTo(EventType.UPDATE_MACRO);
    ...
    assertThat(sagaFromDB.get().getSagaState()).isEqualTo(EventType.UPDATE_MACRO.toString());
  ...
  @Test
  public void testSendMacroUpdateEmail_givenEventAndSagaData_shouldPostEventToEmailApi() throws IOException, InterruptedException, TimeoutException {
    var invocations = mockingDetails(messagePublisher).getInvocations().size();
    var macroPayload = getMacroSagaDataFromJsonString();
    var event = Event.builder()
      .eventType(EventType.UPDATE_MACRO)
      .eventOutcome(EventOutcome.MACRO_UPDATED)
      .sagaId(saga.getSagaId())
      .macroId(macroID)
    ...
    var newEvent = JsonUtil.getJsonObjectFromString(Event.class, new String(eventCaptor.getValue()));
    assertThat(newEvent.getEventType()).isEqualTo(EventType.NOTIFY_MACRO_UPDATE);
    ...
    var sagaFromDB = sagaService.findSagaById(saga.getSagaId());
    assertThat(sagaFromDB).isPresent();
    assertThat(sagaFromDB.get().getSagaState()).isEqualTo(EventType.NOTIFY_MACRO_UPDATE.toString());
    var sagaStates = sagaService.findAllSagaStates(saga);
    assertThat(sagaStates.size()).isEqualTo(1);
    assertThat(sagaStates.get(0).getSagaEventState()).isEqualTo(EventType.UPDATE_MACRO.toString());
    assertThat(sagaStates.get(0).getSagaEventOutcome()).isEqualTo(EventOutcome.MACRO_UPDATED.toString());
  }
